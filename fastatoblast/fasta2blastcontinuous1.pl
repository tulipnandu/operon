$ |  = 1;
my $dir="./Prochlorococcusmarinussubsp.pastorisstr.CCMP1986/";
if(opendir(DIR,$dir))
{
    my @files = readdir(DIR);
    #$files[0]="/." , $files[1]="/.."
    for(my $f=3;$f<=$#files;$f++)
    {
        my $subDir = "".$dir.$files[$f]."/";
        my $input = $subDir."FASTA.txt";
        fasta2blast("blastn","nr",$input,"$subDir$files[$f].csv","CSV");
                print "Done ".($f-2)."/".($#files-2)."\n";
    }
}
