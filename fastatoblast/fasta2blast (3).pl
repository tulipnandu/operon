#!C:/programs/Perl/bin/perl

$pathDir = "/Users/tulip_nandu/Desktop/Anabenacvariabilis";
#print $pathDir."\n";

open FILE, "<", "/Users/tulip_nandu/Desktop/location2.txt" or die $!;
@lines = <FILE>;
#print @lines;
foreach $location (@lines) {
	chomp($location);
	print $location."\n";
	$path = $pathDir.$location."_/FASTA.txt";
	#print $path."\n";
	$outfile = $pathDir.$location."_/result.csv";	
	#print $outfile."\n";
	fasta2blast("blastn","nr",$path,$outfile,"CSV");
	print "DONE!!!...\n\n"
}
close FILE;
...
$req = new HTTP::Request GET => "http://www.ncbi.nlm.nih.gov/blast/Blast.cgi?CMD=Get&FORMAT_TYPE=$formatType&RESULTS_FILE=on&FORMAT_OBJECT=Alignment&ALIGNMENT_VIEW=Tabular&RID=$rid";
$response = $ua->request($req);
